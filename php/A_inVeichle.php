...
function insert_Veichle($name, $brand, $type_engine, $type_model, $passenger, 
                        $weight, $power, $price, $width, $length, $height, $db_connection) 
{
    if ($stmt = mysqli_prepare($db_connection, "INSERT INTO modello
                (idModello, nome, marca, tipoMotore, tipoModello,
                    ...
                    lunghezza, altezza) VALUES (null, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
        mysqli_stmt_bind_param($stmt, "ssssidddddd", $name,  $brand, $type_engine, $type_model, $passenger, 
...
$successful = insert_Veichle($name, $brand, $type_engine, $type_model, $passenger, 
                    $weight, $power, $price, $width, $length, $height, $con);
    mysqli_close($con);
    ...
    } else 
    {
		...
		header('Location: A_formVeichle.php?err=5');
	}
